...
pub fn jsonl_deser<T: DeserializeOwned, R: BufRead>(
    reader: R,
...
pub fn label_to_index(label: &serde_json::Value, labels: &[String]) -> Result<usize> {
    match label {
        serde_json::Value::Number(n) => {
            let i = n
                .as_u64()
                .ok_or_else(|| anyhow::anyhow!("label index must be non-negative integer"))?
                as usize;
            if i >= labels.len() {
                bail!("label index {} out of range {}", i, labels.len());
            }
            Ok(i)
        }
        serde_json::Value::String(s) => labels
            .iter()
            .position(|t| t == s)
            .ok_or_else(|| anyhow::anyhow!("label '{}' not in labels", s)),
...
pub fn resolve_label_index(
    ...
    if let Some(i) = label_index {
        if i >= k {
            bail!("label_index {} out of range {}", i, k);
        }
        return Ok(i);
    }
    if let Some(lbl) = label {
        if let Some(canon) = canon_labels {
            return label_to_index(lbl, canon);
        }
        match lbl {
            serde_json::Value::Number(n) => Ok(n
                .as_u64()
                .ok_or_else(|| anyhow::anyhow!("label number invalid"))?
                as usize),
